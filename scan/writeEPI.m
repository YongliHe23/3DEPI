...
%   ovs          boolean  [t/F] if use OVS pulse or not
%   ovs_path     string   path to the OVS pulse MAT-file
%
% Outputs:
%   gro 
%   adc
...
%     'ftype', ftype,...
%     'noRfOffset', strcmp(type, '3D'));   % don't shift slice (slab) for 3D);      % filter design. 'ls' = least squares
% 
...
[rf,gzRF, gzRF_r] = mr.makeSincPulse(alpha/180*pi, sys, 'Duration', 4e-3, 'SliceThickness', slThick, 'apodization',0.5,'timeBwProduct',8);
...
if strcmp(type, 'SE')
    ...
    minTE = mr.calcDuration(gzRF_r)+mr.calcDuration(gzRF) - mr.calcDuration(rf)/2 - rf.delay + mr.calcDuration(gxPre) + ...
            (kyIndAtTE-0.5) * mr.calcDuration(gro); %sinc-alpha
    % minTE = mr.calcDuration(rf)/2 - rf.delay + mr.calcDuration(gxPre) + ...
    %     (kyIndAtTE-0.5) * mr.calcDuration(gro);
    ...
    if arg.ovs
        minTR = arg.fatSat*(mr.calcDuration(rfsat) + mr.calcDuration(gxSpoil)) + ...
            mr.calcDuration(rf) + TEdelay + mr.calcDuration(gzRF_r)+...
            mr.calcDuration(gxPre) + etl*mr.calcDuration(gro) + mr.calcDuration(gxSpoil)+...
            mr.calcDuration(rf_beta)+mr.calcDuration(gxSpoil);
    else
        minTR = arg.fatSat*(mr.calcDuration(rfsat) + mr.calcDuration(gxSpoil)) + ...
            mr.calcDuration(rf) + TEdelay + mr.calcDuration(gzRF_r)+...
            mr.calcDuration(gxPre) + etl*mr.calcDuration(gro) + mr.calcDuration(gxSpoil)+mr.calcDuration(gxSpoil);
    end
    if strcmp(type, '3D')
    ...
    for p = IP
        ...
        if arg.fatSat
            ...
            if ~arg.doNoiseScan
                ...
                seq.addBlock(gx_beta,gy_beta,gz_beta,mr.makeLabel('SET', 'TRID', segmentID));%beta gr
                seq.addBlock(gxSpoil,gzSpoil);
                %seq.addBlock(gzRF); %excitation gr
            end
            if arg.RFspoil
                rf_inc = mod(rf_inc+rfSpoilingInc, 360.0);
                rf_phase = mod(rf_phase+rf_inc, 360.0);
